...
float norm(glm::vec4 v)
{
    ...
    return sqrt( vx*vx + vy*vy+ vz*vz ); /*o que falta para definir norma Euclidiana */
}
...
glm::mat4 Matrix_Rotate(float angle, glm::vec4 axis)
{
    ...
    return Matrix(
        ...
        (vx*vx)*(1-c)+c , (vx*vy)*(1-c)-vz*s , (vx*vz)*(1-c)+vy*s , 1.0f ,  // LINHA 1
        (vx*vy)*(1-c)+vz*s , (vy*vy)*(1-c)+c , (vy*vz)*(1-c)-vx*s , 1.0f ,  // LINHA 2
...
glm::vec4 crossproduct(glm::vec4 u, glm::vec4 v)
{
    ...
    return glm::vec4(
        ...
        u1*v2 - u2*v1, // o terceiro coeficiente de um produto vetorial
...
glm::vec4 vector_v(glm::vec4 view_vector, glm::vec4 up_vector)
{
    glm::vec4 w = -view_vector/norm(view_vector);
